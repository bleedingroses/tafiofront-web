...
class analisaOverStok extends Resource
{    
    use tabs;
...
if($produk->stokTotal<$produk->stokMin*1.6)
$ambil->forget($baris);
else
{

if($produk->penjualanHarian>0)
$produk->habis=floor($produk->stokTotal/$produk->penjualanHarian);    
else
$produk->habis='blm laku';    
...
$hasil=$ambil->sortByDesc('habis');
        ...
        $this->fields = [
          ...
          (new noForm)->make('habis')->judul('brapa hari <br>habis'),
...
produkModel::find(ambil('analisaPoLagi'))->update(['abaikan'=>$abaikan]);
